...
import { getCorrectFormBullets, getCheckpoints, type CheckpointHighlight } from '../lib/biomechanicsUtils';
...
export default function Biomechanical3DViewer({ exerciseName, isRest = false }: Props) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-stretch justify-between bg-slate-950 rounded-[32px] border border-slate-900 shadow-2xl overflow-hidden relative group">
      ...
      <div className="p-4 flex justify-between items-start z-10 select-none bg-slate-950/20 border-b border-slate-900/60 md:backdrop-blur-">
      ...
      >
        ...
        {/* Help Tip Overlay */}
        <div className="absolute top-2 left-1/2 -translate-x-1/2 bg-slate-950/80 border border-slate-900 text-[8px] font-sans px-3 py-1 rounded-full text-slate-300 pointer-events-none flex items-center gap-1.5 md:backdrop-blur- shadow-md">
        ...
        {showFormCheck && (
          <motion.div
            ...
            className="absolute top-4 right-4 z-20 w-44 bg-slate-950/90 border border-slate-900 rounded-2xl p-3 md:backdrop-blur- shadow-2xl pointer-events-auto select-none space-y-2 text-[10px]"
          >
      ...
      <div className="px-4 py-3 bg-slate-950 border-t border-slate-900 z-10 space-y-2 md:backdrop-blur-">
